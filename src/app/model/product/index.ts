...
class ProductModel {
    ...
    async GetRelatedProduct(category_id: number, limit: number) {
        try {
            let queryStr = `
                select                 
    ...
    async GetProductTrademarks(category_id: string) {
        try {
            const queryStr = `
    ...
    //Lấy tất cả sản phẩm
    async GetProducts(startIndex: number, limit: number) {
        try {
            const queryStr = `
            select                 
    ...
    async GetProductsByCategoryName(startIndex: number, limit: number, categoryName: string) {
        try {
            const queryStr = `
            select                 
    ...
    async GetProductBySlug(slug: string) {
        try {
            ...
            const product = await ReturnItem(queryProduct, errorMsg);
            ...
            const toppings = await ReturnItems(queryTopping, errorMsg);
    ...
    async LoadProductCoffeeOfHome(categoryName: string, productNames: string[], limit: number) {
        try {
    ...
    async FillterProduct(
        ...
        const trademarks = trademark?.split(',');
        const trademarkStr = trademarks?.map((item) => `'${item}'`);
        let whereStr;
        ...
        else if (category_id) whereStr = `where category.category_id in ( ${category_id})`;
        else if (minPrice && maxPrice) whereStr = `where price >= (${minPrice}) and price < ( ${maxPrice})`;
        else if (minPrice && !maxPrice) whereStr = `where price >= (${minPrice})`;
        else if (trademark) whereStr = `where trademark in  (${trademarkStr})`;
    ...
    async GetColectionCategory() {
        try {
            ...
            let colectionCategorys = [] as colectionCategoryType[];

            const categorys = (await ReturnItems(queryStr, errorMsg)) as categoryType[];
            ...
            `;
                ...
                const products = (await ReturnItems(queryStr1, errorMsg)) as product[];
    ...
    async LoadTypicalProducts(categoryString: string) {
        try {
            ...
            for (let i = 0; i < categorys.length; i++) {
                ...
                const product = await ReturnItems(query, errorMsg);
    ...
    //sort
    async Sort(type: string, key: string, page: number, limit: number, slug: string) {
        const query =
            slug !== 'all'
                ? `
            select                 
    ...
    `;
        ...
        return await ReturnItems(query, err);
    }
}
