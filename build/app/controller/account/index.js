        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (!req.params.customer_id)
                    return res.status(404).json({ msg: 'Không tìm thấy id' });
                if (!req.file)
                    return res.status(404).json({ msg: 'Không tìm thấy file!' });
                const result = yield AccountModel.UpdateAvatar(req.params.customer_id, req.file.path);
                if (!result)
